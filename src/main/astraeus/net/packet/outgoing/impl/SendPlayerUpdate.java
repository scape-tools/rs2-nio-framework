...
public final class SendPlayerUpdate extends OutgoingPacket {
      ...
      public PacketWriter encode(Player player) {
            ...
            for (Player other : World.getPlayers()) {
                  ...
                  if (other.getLocalPlayers().size() >= 79
       ...
       * 
       * @param writer The writer used to write and store data.
       */
      public void addPlayer(Player player, Player other, PacketWriter writer) {
            player.getLocalPlayers().add(other);
            writer.writeBits(11, other.getSlot()).writeBit(true).writeBit(true)
       ...
       * 
       * @param writer The buffer to store data.
       */
       ...
       * 
       * @param writer The writer that constructs a packet and stores the data.
       * 
       * @param forceAppearance The flag that determines if this player should be forcefully
       *        updated.
       */
      public void appendUpdates(Player player, PacketWriter buffer, boolean forceAppearance,
                  boolean noChat) {
            synchronized (player) {
                  ...
                  if (mask >= 0x100) {
                        mask |= 0x40;
                        buffer.write((mask & 0xFF)).write((mask >> 8));
                  } else {
                        buffer.write(mask);
                  }
